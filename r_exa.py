...
class LinearProgrammingSolver:
    ...
    def simplex_method(self, c, A, b, signs, var_constraints):
        ...
        all_variable_names = variable_names.copy()
        display_indices = [i for i, name in enumerate(all_variable_names) if name.startswith('X') or name.startswith('S')]
        
        # Mostrar el tableau inicial
        self.output_text.insert(tk.END, "Tabla Inicial:\n")
        self.display_tableau(tableau, c_extended, basis, all_variable_names, display_indices)
        ...
        while iteration < max_iterations:
            ...
            self.display_tableau(tableau, c_extended, basis, all_variable_names, display_indices, cj_zj=cj_zj)
            ...
            self.output_text.insert(tk.END, f"Variable entrante: {all_variable_names[entering]}\n")
            ...
            self.output_text.insert(tk.END, f"Variable saliente: {all_variable_names[basis[leaving]]}\n")
    ...
    def display_tableau(self, tableau, c_extended, basis, all_variable_names, display_indices, cj_zj=None, ratios=None, entering=None):
        num_vars = len(display_indices)
        ...
        for idx in display_indices:
            var_name = all_variable_names[idx]
            title_row += f"  {var_name:^8}|"
        title_row += "  Solución |\n"
        ...
        for i in range(len(tableau)):
            row = tableau[i]
            base_var_idx = basis[i]
            base_var_name = all_variable_names[base_var_idx]
            row_str = f"| {base_var_name:^6}|"
            for idx in display_indices:
                val = row[idx]
                row_str += f" {val:>8.2f}|"
            row_str += f" {row[-1]:>9.2f}|\n"
        ...
        if cj_zj:
            ...
            for idx in display_indices:
                val = cj_zj[idx]
                row_str += f" {val:>8.2f}|"
            row_str += f" {z_value:>9.2f}|\n"
            self.output_text.insert(tk.END, row_str)
            self.output_text.insert(tk.END, "+------+")
            self.output_text.insert(tk.END, "----------" * num_vars + "+")
            self.output_text.insert(tk.END, "-----------+\n")
        
        # Mostrar razones si corresponde
        if ratios and entering is not None:
            ratio_row = f"| Ratio |"
            for idx in display_indices:
                if idx == entering:
                    ratio_str = ""
                    for i in range(len(ratios)):
                        if tableau[i][entering] > 1e-5:
                            ...
                            break
                    ratio_row += f" {ratio_str:^8}|"
                else:
                    ratio_row += "          |"
            ratio_row += "           |\n"
            self.output_text.insert(tk.END, ratio_row)
            self.output_text.insert(tk.END, "+------+")
            self.output_text.insert(tk.END, "----------" * num_vars + "+")
            self.output_text.insert(tk.END, "-----------+\n")
    
    # Métodos big_m_method y two_phase_method con ajustes similares
    def big_m_method(self, c, A, b, signs, var_constraints):
        ...
        for i in range(num_constraints):
            ...
            if signs[i] == "<=":
                ...
                artificial[i] = 1
                c_extended.append(-M)
                # No agregar variable artificial al nombre de variables para imprimir
        ...
        all_variable_names = variable_names + [f"A{i+1}" for i in range(len(artificial_vars))]
        display_indices = [i for i, name in enumerate(all_variable_names) if name.startswith('X') or name.startswith('S')]
    ...
    def two_phase_method(self, c, A, b, signs, var_constraints):
        ...
        self.display_tableau(tableau, c_phase1, basis, all_variable_names, display_indices)
        ...
        while iteration < max_iterations:
            ...
            # Mostrar el tableau con las ganancias relativas
            self.display_tableau(tableau, c_phase1, basis, all_variable_names, display_indices, cj_zj=cj_zj)
        ...
        variables_to_keep = [i for i in range(len(all_variable_names)) if i not in artificial_vars]
        variable_names = [all_variable_names[i] for i in variables_to_keep]
        ...
        self.display_tableau(tableau, c_extended_phase2, basis, all_variable_names, display_indices)
        ...
        while iteration < max_iterations:
            ...
            # Mostrar el tableau con las ganancias relativas
            self.display_tableau(tableau, c_extended_phase2, basis, all_variable_names, display_indices, cj_zj=cj_zj)
